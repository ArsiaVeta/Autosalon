package ru.arslanova.orderservice.api.errors;
...
public class GlobalExceptionHandler {
    ...
    public ErrorResponse handleStorageUnavailable(
            StorageUnavailableException exception,
            HttpServletRequest request){
        log.error("StorageService unavailable: {}", exception.getMessage());
        return new ErrorResponse(
                Instant.now(), 503, "SERVICE_UNAVAILABLE",
                exception.getMessage(),
                request.getRequestURI()
        );
    }
